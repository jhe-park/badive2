        ...
        <div className="w-full h-full">
          <div className="relative w-full h-full flex justify-center items-center">
            <div className="xl:max-w-[500px] md:max-w-[250px] max-w-[72px] aspect-[5/6] relative max-h-[600px]">
              <img loading="lazy" src={slides[currentSlide].image} alt={`Slide ${currentSlide + 1}`} className="w-full h-full object-cover" />
                  ...
                  {slides.map((_, dotIndex) => (
                    <button
                      ...
                      className={`w-1 h-1 md:w-3 md:h-3 rounded-full transition-all duration-300 ${
                        currentSlide === dotIndex ? 'bg-white scale-110' : 'bg-gray-400 hover:bg-gray-200'
                      }`}
