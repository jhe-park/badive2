...
export default function ModalForDetailInformation({ isOpen: isOpenProps, onOpenChange: onOpenChangeProps, onClose: onCloseProps }) {
  ...
  //       ?.filter(reservation => reservation.status !== '취소완료' && rawData.some(slot => slot.id === reservation.time_slot_id && slot.date === date))
  //       .map(reservation => {
  ...
  //         status: !slot.available ? 2 : remainingSpots < selectedResult.noParticipants ? 2 : 0,
  ...
  //             if (schedules[startIndex + i].status !== 2) {
  //               // 예약불가가 아닌 경우에만
  ...
  //     if (nextTwoSlots.length === 2 && nextTwoSlots.every(slot => slot.status === 0)) {
  //       // 3개의 연속된 셀 키 생성
  //       const selectedCells = [`${timeIndex}-${dateIndex}`, `${timeIndex + 1}-${dateIndex}`, `${timeIndex + 2}-${dateIndex}`];
  //       setSelectedCell(selectedCells);

  //       // 선택된 첫 번째 슬롯 찾기
  //       const selectedSlot = data.find(slot => slot.date === tableData[dateIndex].date && `${slot.start_time}~${slot.end_time}` === currentSlot.time);

  //       // 두 번째 슬롯 찾기
  //       const secondSlot = data.find(
  //         slot => slot.date === tableData[dateIndex].date && `${slot.start_time}~${slot.end_time}` === tableData[dateIndex].schedule[timeIndex + 1].time,
  //       );

  //       // 세 번째 슬롯 찾기
  //       const thirdSlot = data.find(
  //         slot => slot.date === tableData[dateIndex].date && `${slot.start_time}~${slot.end_time}` === tableData[dateIndex].schedule[timeIndex + 2].time,
  //       );
  ...
  return (
    <>
      <Modal
        classNames={{ base: 'z-9999 max-h-[10vh] bg-red-100' }}
        // size="full"
        size="lg"
        isOpen={isOpenProps}
        onClose={() => {}}
