...
export default function Calendar() {
  ...
  const getPrograms = async () => {
    const { data: programs, error: programsError } = await supabase.from('program').select('*').eq('available', true);
    if (programsError) {
      console.log('프로그램 조회 중 에러 발생:', programsError);
      return;
    }
    if (programs && selectedInstructor) {
      const filteredPrograms = programs.filter(program => program.instructor_id === selectedInstructor?.id);
      ...
      <div className="flex flex-col md:flex-row w-full justify-start gap-x-4 gap-y-2">
        ...
        >
          {instructors.map(instructor => (
            <SelectItem onPress={() => handleInstructorClick(instructor)} key={instructor.name} value={instructor.name}>
              {instructor.name}
            </SelectItem>
          ))}
        </Select>
        ...
        >
          <TableHeader>
            <>
              ...
              {instructors.map(instructor => (
                <TableColumn key={instructor.id} className="text-center w-[100px]">
                  {instructor.name}
                </TableColumn>
              ))}
            </>
