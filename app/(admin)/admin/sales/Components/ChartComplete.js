...
export default function ChartComplete() {
    ...
    if (error) {
      ...
      const salesByCategory = programList.map(category => {
        const filteredReservations = data.filter(
          reservation => reservation.time_slot_id.program_id.category === category
        );
        
        const totalSales = filteredReservations.reduce(
  ...
  const getTourInput = async () => {
      ...
      if (error) {
        ...
        setMonthlySales(prevMonthlySales => 
          prevMonthlySales.map(monthlySale => 
            monthlySale.name === yearMonth 
              ? { ...monthlySale, sales: monthlySale.sales + (data.amount || 0) }
              : monthlySale
          )
        );
      }
    ...
    setDetailSales(prevDetailSales => {
      ...
      return [
        ...prevDetailSales,
        { key: uniqueKey, name: "다이빙투어", sales: totalAmount }
      ];
    });
    ...
    <div className="w-full h-full flex flex-col gap-4 ">
      <div className="w-full flex flex-row gap-4">
        <Select  label="시작년도" selectedKeys={[startYear]} onChange={(e) => setStartYear(e.target.value)} >
          ...
          <SelectItem key="2026" value="2026">2026년</SelectItem>
        </Select>
        <Select  label="시작월" selectedKeys={[startMonth]} onChange={(e) => setStartMonth(e.target.value)} >
        <SelectItem key="1" value="1">1월</SelectItem>
          <SelectItem key="2" value="2">2월</SelectItem>
          <SelectItem key="3" value="3">3월</SelectItem>
          <SelectItem key="4" value="4">4월</SelectItem>
          <SelectItem key="5" value="5">5월</SelectItem>
          <SelectItem key="6" value="6">6월</SelectItem>
          <SelectItem key="7" value="7">7월</SelectItem>
          <SelectItem key="8" value="8">8월</SelectItem>
          <SelectItem key="9" value="9">9월</SelectItem>
          <SelectItem key="10" value="10">10월</SelectItem>
          <SelectItem key="11" value="11">11월</SelectItem>
          <SelectItem key="12" value="12">12월</SelectItem>
        </Select>
