...
export default function TableData() {
  ...
  const getReservation = async () => {
    let query = supabase.from('reservation').select('*,time_slot_id(*,instructor_id(*),program_id(*))').eq('status', '예약확정');
    ...
    if (selectedProgram) {
      query = query.eq('time_slot_id.program_id.category', selectedProgram);
    }
    if (selectedInstructor) {
      query = query.eq('instructor_id', parseInt(selectedInstructor));
    }

    let { data, error } = await query;

    if (selectedProgram) {
      data = data.filter(item => item.time_slot_id && item.time_slot_id.program_id !== null);
    }
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4">
      ...
      <div className="flex flex-col gap-4 md:flex-row">
        ...
        <Select selectedKeys={[selectedProgram]} onChange={e => setSelectedProgram(e.target.value)} className="w-full md:w-1/3" label="프로그램">
        ...
        <Select selectedKeys={[selectedInstructor]} onChange={e => setSelectedInstructor(e.target.value)} className="w-full md:w-1/3" label="강사">
