...
export default function SubmitListPage({ params }) {
  ...
  const getInstructor = async () => {
    ...
    const { data: instructor, error } = await supabase.from('request').select('*,tour_id(*)').eq('id', id).single();
    ...
    <div className="w-full h-full flex flex-col gap-4">
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input variant="bordered" label="투어명" value={instructor.tour_id.title || ''}></Input>
          <Input variant="bordered" label="투어일자" value={instructor.tour_id.date || ''}></Input>
