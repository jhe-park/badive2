...
import { Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Select, SelectItem, Button } from '@heroui/react';
...
export default function TourTable({ member }) {
  ...
  const getRequest = async () => {
    const { data, error } = await supabase.from('request').select('*,tour_id(*)').eq('user_id', member?.id);
  ...
  const handleStatusChange = async (tour, newStatus) => {
    try {
      const { data, error } = await supabase.from('request').update({ is_end: newStatus }).eq('id', tour.id);

      if (error) throw error;

      // 로컬 상태 업데이트
      setTours(tours.map(t => (t.id === tour.id ? { ...t, is_end: newStatus } : t)));
    } catch (error) {
  ...
  const updateTourStatus = async tour => {
    ...
    const tourDate = new Date(tour.tour_id.date.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3'));
  ...
  return (
    <div className="mb-24 flex h-full w-full flex-col gap-y-6">
      ...
      <Table classNames={{ wrapper: 'p-0 ' }} aria-label="Example static collection table" shadow="none">
        <TableHeader>
          ...
          <TableColumn className="w-1/4 text-center">투어일일</TableColumn>
          ...
          {tours.map(tour => (
            <TableRow key={tour.id}>
              ...
              <TableCell className="whitespace-nowrap text-center">
                ...
                >
                  <SelectItem className="text-center text-red-500" value="투어종료" key="투어종료">
                    투어종료
                  </SelectItem>
                  <SelectItem className="text-center text-green-500" value="투어중" key="투어중">
                    투어중
                  </SelectItem>
                  <SelectItem className="text-center text-blue-500" value="투어예정" key="투어예정">
                    투어예정
                  </SelectItem>
                </Select>
              </TableCell>
