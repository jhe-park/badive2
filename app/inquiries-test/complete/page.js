...
export default async function page({ searchParams }) {
  const { orderId, instructor_id, time_slot_id, user_id, participants, paymentKey, amount, pay_type, program_id } = searchParams;
  ...
  try {
    ...
    if (orderId) {
      // 먼저 예약 존재 여부 확인
      const { data: existingReservation } = await supabase.from('reservation').select('*').eq('order_id', orderId).single();
      ...
      if (!existingReservation) {
        ...
        for (const slotId of timeSlotIds) {
          const { data: timeSlot } = await supabase.from('timeslot').select('*').eq('id', slotId).single();
