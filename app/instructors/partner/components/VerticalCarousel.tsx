  ...
  return (
    <div className="relative w-full h-1/2 mx-auto">
      <div className="absolute -left-8 top-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-2">
        <button onClick={scrollPrev} className="text-white hover:text-blue-500 transition-colors" aria-label="이전 슬라이드">
        ...
        {images.map((_, index) => (
          <button
            key={index}
            className={`w-3 h-6 rounded-full transition-all duration-300 ${selectedIndex === index ? 'bg-blue-500 scale-125' : 'bg-white'}`}
            onClick={() => emblaApi?.scrollTo(index)}
            aria-label={`슬라이드 ${index + 1}로 이동`}
          />
        ))}

        <button onClick={scrollNext} className="text-white hover:text-blue-500 transition-colors" aria-label="다음 슬라이드">
      ...
      <div className="overflow-hidden h-full" ref={emblaRef}>
        <div className="flex flex-col h-full">
          {images.map((image, index) => (
            <div key={index} className="relative flex-[0_0_100%] h-[400px] w-[600px] group cursor-pointer" onClick={() => handleImageClick(image)}>
